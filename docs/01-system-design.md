...
`0.40 × 需求匹配 + 0.15 × 模型共识 + 0.20 × 数据可信度 + 0.20 × 性价比 + 0.05 × 数据时效性`

其中“模型共识”先按模型中心化评分尺度，再计算平均分和分歧度，避免某个小模型整体打高分时支配排序；共识分仍限制在规则结果附近。“数据可信度”同时考虑官网来源、字段完整度、旗舰店核验、价格证据和手工价格的未核验状态。权重集中定义在推荐服务中，便于消融实验。

价格按可信级别处理：已审核且带证据的官方店价格优先参与预算判断；没有已审核价时才使用手工参考价，并降低数据可信分、在界面和解释中明确标注；待复核数据不会冒充官方价格。
