...
|---|---|---|
| `brands` | 品牌与官网 | 品牌名唯一 |
| `phone_models` | 型号与硬件参数 | 同一品牌型号唯一；发布日期/销售状态联合索引 |
| `phone_variants` | RAM、存储与发售价 | 型号 + RAM + 存储唯一 |
| `platform_listings` | 平台、官方店铺和商品链接 | 平台 + 商品ID + SKU文字唯一 |
| `price_snapshots` | 普通价、活动价、国补、百亿补贴历史 | 商品与采集时间索引；价格允许为空但不能猜测 |
...
- `displayed_gov_price`：页面明确展示的国补后价格；
- `estimated_gov_price`：系统按规则估算的价格，永远与展示价分开；
- `billion_subsidy_price`：平台百亿补贴频道价格；
- `promotion_stackable`：优惠能否叠加，不确定时必须为 `unknown`；
- `evidence_text_path` / `screenshot_path`：采集证据路径。

推荐预算比较优先使用无需资格的公开活动价；条件价格只做提示，避免把没有资格的用户误判为预算内。

`phone_models.image_url` 保存品牌官网产品图地址，`image_source_url` 保存能够证明图片归属的官网页面。页面不生成或拼接虚假手机图；图片缺失或加载失败时明确显示缺失状态。

## 4. 增删改查策略

后台支持品牌、型号、版本、商品链接和价格快照的新增与查询；型号、状态等可修改；删除采用 `is_active=false` 或 `discontinued`。价格不修改旧快照，而是新增一条记录，从而可以解释“推荐当时为什么是这个价格”。
